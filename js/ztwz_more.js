...
let options = {
	...
	"callBack": function (result) {
		...
		result.forEach(function(item,index){
			if(index == 0 ){
				cHtml +='<li class="clearfix"><div class="fl"><img src="'+ item.content +'"></img></div>'
				cHtml +='<div class="fr content"><a href="../subPage/dzyd_more.html"><p>'+ item.title +'</p></a></div></li>'
			} else {
				cHtml +='<li class="clearfix"><div class="fl"><img src="'+ item.content +'"></img></div>'
				cHtml +='<div class="fr content"><a href="'+ item.path +'"><p>'+ item.title +'</p></a></div></li>'
			}
...
$(function () {
	...
	var tzggMoreLanguage = {
		titleCn:["专题网站",],
		titleEn:["Special website",],
		titleBreadCn:['当前位置：<a href="../index.html">首页</a>&bull;专题网站'],
		titleBreadEn:['Current location:<a href="../index.html">Home</a>&bull;Special website'],
	
	}

	if(languageStatus == 1){
		//中文
		$("h2[name = ztwzMoreTitle]").text(tzggMoreLanguage["titleCn"])
		$("span[name = ztwzMoreBread]").html(tzggMoreLanguage["titleBreadCn"])
	}else {
		//英文
		$("h2[name = ztwzMoreTitle]").text(tzggMoreLanguage["titleEn"])
		$("span[name = ztwzMoreBread]").html(tzggMoreLanguage["titleBreadEn"])
	}
