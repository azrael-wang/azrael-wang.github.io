    ...
    // console.log(result)
		for (var i = 0; i < result.length; i++) {
			...
			if(result[i].type == 268){
				cHtml +='<li><a href="../subPage/sxhyInfo.html?id='+ result[i].standby1 +'"><p class="date"> '+ Year +'</p><p class="title">'+ result[i].name +'</p></a></li>'
			}else {
				cHtml +='<li><a href="../subPage/xsjz_info.html?id='+ result[i].id +'"><p class="date"> '+ Year +'</p><p class="title">'+ result[i].name +'</p></a></li>'
			}
...
paging(languageStatus);
	...
	if(languageStatus == 1){
		//中文
		$("h2[name = xsTitle]").text(xsMoreLanguage["titleCn"])
		$("span[name = xsBread]").html(xsMoreLanguage["titleBreadCn"])
	}else {
		//英文
		$("h2[name = xsTitle]").text(xsMoreLanguage["titleEn"])
		$("span[name = xsBread]").html(xsMoreLanguage["titleBreadEn"])
	}
