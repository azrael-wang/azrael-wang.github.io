$(function () {
      ...
      success: function (res) {
        ...
        if (res.code === 0) {
          ...
          if (res.page.list.length != 0) {
            var result = res.page.list
            var imgStr = ""
            for (var i = 0; i < result.length; i++) {
              var content = result[i].title.replace(/&amp;nbsp;/g, "&#10;")
              content = content.substr(0, 30).replace(/&amp;nbsp;/g, "&#10;")
              if (i == 0) {
                if (result[i].imgurl == null || result[i].imgurl == "") {
                  imgStr += "<img  src='../img/noticeAdvance.png'></img><span>" + content + "</span>"
                } else {
                  imgStr += "<img  src='" + result[i].imgurl + "'></img><span>" + content + "</span>"
                }
  ...
  //导航列表
  var NavObj = {
    "CN": ["日常管理", "教学科研", "规章制度", "下载中心", "联系我们"],
    "EN": ["Daily management", "Teaching and research", "Regulatory Framework", "DownCenter", "Connect us"],
    "titleCn": ["重要通知", "通知公告"],
  ...
  function NavList(NavObj) {
    ...
    if (languageStatus == 1) {
      ...
      NavObj["CN"].forEach(function (item, index) {
        str += "<li><a href='../subPage/jiaozhigong.html?index=" + index + "'><img src='../img/jzgTz" + (index + 1) + ".png'></img><span>" + item + "</span></a></li>"
      })
      //面包屑
