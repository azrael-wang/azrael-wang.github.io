...
let options = {
	...
	"callBack": function(result) {
		...
		var path = 0
		switch (true) {
			case window.getType == 134:
				path = 0
				break;
			case window.getType == 135:
				path = 1
				break;
			default:
				path = 2
		...
		for (var i = 0; i < result.length; i++) {
			...
			cHtml += '<li><a href="./xiaoyuansh.html?index=' + path + '&type'+ window.getType +':' + i + '"><img src="' + (result[i].remark) +
				'" alt=""></a>'
			//处理数据
		}

		$(".sub_content_img_style .clearfix").html(cHtml); //将数据增加到页面中
...
$(function() {
	...
	function paging() {
		var getTypeStr = window.location.search
		var typeIndex = getTypeStr.indexOf("=")
		window.getType = getTypeStr.slice(typeIndex + 1)
		$.ajax({
			type: "POST",
	...
	if (languageStatus == 1) {
		//中文
		$("h2[name = jyMoreTitle]").text(jyMoreLanguage["titleCn"])

		$("span[name = jyMoreBread]").html(jyMoreLanguage["titleBreadCn"])
	} else {
		//英文
		$("h2[name = jyMoreTitle]").text(jyMoreLanguage["titleEn"])
		$("span[name = jyMoreBread]").html(jyMoreLanguage["titleBreadEn"])
	}
