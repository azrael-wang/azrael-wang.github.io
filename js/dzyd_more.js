$(function () {
	...
	var options = {
		...
		cHtml +='<li><a href="../subPage/dzyd_info.html?id='+ result[i].id +'"><p class="date"> '+ Year +'</p><p class="title">'+ result[i].title +'</p></a></li>'
				//处理数据
			}
	...
	if (languageStatus == 1) {
		...
		//中文
		$("h2[name = tzggMoreTitle]").text(dzydLanguage["titleCn"])
		$("span[name = tzggMoreBread]").html(dzydLanguage["titleBreadCn"])
	} else {
		//英文
		$("h2[name = tzggMoreTitle]").text(dzydLanguage["titleEn"])
		$("span[name = tzggMoreBread]").html(dzydLanguage["titleBreadEn"])
	}
