...
let options = {
	...
	"callBack": function (result) {
      ...
      var Year = result[i].createTime.split(" ")[0]
		...
		cHtml +='<li><a href="../subPage/xysh_info.html?id='+ result[i].id +'"><p class="date"> '+ Year +'</p><p class="title">'+ result[i].title +'</p></a></li>'
			//处理数据
		}
...
$(function () {
	...
	if(languageStatus == 1){
		//中文
		$("h2[name = xyshTitle]").text(xyshMoreLanguage["titleCn"])
		$("span[name = xyshBread]").html(xyshMoreLanguage["titleBreadCn"])
	}else {
		//英文
		$("h2[name = xyshTitle]").text(xyshMoreLanguage["titleEn"])
		$("span[name = xyshBread]").html(xyshMoreLanguage["titleBreadEn"])
	}
